...
def _fill_general_templates(title, component_list, report_type='html'):
    ...
    jinjaout = template.render(
        title = title, component_list=component_list
    )
    return jinjaout


def generate_full_report_for_recorder(
        exp_id=None, exp_name=None, record_name=None, recorder_id=None,
        report_contents=('Backtest Performance', 'Model Evaluation'), dynamic_figure=True
):
    """
    ...
    """
    assert (
        (exp_id is not None or exp_name is not None) and (recorder_id is not None or record_name is not None)
        ...
        dict(
            ...
            content=[
                dict(
                    header='Run Params',
                    type='itemlist',
                    content=recorder.list_params()
                ),
                dict(
                    header='Tags',
                    type='itemlist',
                    content=recorder.list_tags()
                ),
            ]
        )
    ]
    ...
    if 'Model Evaluation' in report_contents:
        ...
        pred_label = pd.merge(
            label_df.iloc(axis=1)[0].rename('label'), pred_df.iloc(axis=1)[0].rename('score'),
